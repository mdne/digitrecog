...
class Dataset():
	...
	def model1(self, X, w_h, w_h2, w_o, p_keep_input, p_keep_hidden): # this network is the same as the previous one except with an extra hidden layer + dropout
	...
	def train1(self):
		...
		w_h = self.init_weights([16, 625])
		w_h2 = self.init_weights([625, 625])
		w_o = self.init_weights([625, 10])

		p_keep_input = tf.placeholder("float")
		p_keep_hidden = tf.placeholder("float")
		py_x = self.model1(X, w_h, w_h2, w_o, p_keep_input, p_keep_hidden)

		cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(py_x, Y))
		train_op = tf.train.GradientDescentOptimizer(0.05).minimize(cost)#tf.train.RMSPropOptimizer(0.001, 0.9).minimize(cost)
	...
	def model3(self, X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden):
	...
	def train3(self):
		...
		p_keep_hidden = tf.placeholder("float")
		py_x = self.model3(X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden)
